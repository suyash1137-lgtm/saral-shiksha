      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
        {filtered.map((c) => {
          ...
          return (
            <div
              key={c.id}
              className={`bg-white rounded-2xl border ${styling.ring} p-6 shadow-sm hover:shadow-md transition-all flex flex-col justify-between gap-5`}
            >
              <div className="flex flex-col gap-3">
                <div className="flex items-start justify-between gap-2">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="font-bold text-gray-900 text-lg leading-tight">{c.title}</h2>
                  ...
                  <span
                    className={`text-xs px-2.5 py-0.5 rounded-full font-semibold border ${
                      levelColors[c.level] || 'bg-gray-100 text-gray-600'
                    }`}
                  >
                    {c.level}
                  </span>
              ...
              <div className="flex flex-col gap-4 border-t border-gray-100 pt-4">
                ...
                <div className="flex flex-wrap gap-1">
                  {c.tags.slice(0, 3).map((tag) => (
                    <span
                      key={tag}
                      className="text-xs bg-gray-100 text-gray-600 font-medium px-2 py-0.5 rounded-md"
                    >
                      #{tag}
                    </span>
                  ))}
